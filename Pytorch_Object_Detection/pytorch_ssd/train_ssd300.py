...
from pytorch_ssd import transform
from pytorch_ssd.my_dataset import VOC2012DataSet
from pytorch_ssd.src.ssd_model import SSD300, Backbone
import pytorch_ssd.train_utils.train_eval_utils as utils
from pytorch_ssd.train_utils.coco_utils import get_coco_api_from_dataset
...
def main(parser_data):
    ...
    if len(train_loss) != 0 and len(learning_rate) != 0:
        from pytorch_ssd.plot_curve import plot_loss_and_lr
        plot_loss_and_lr(train_loss, learning_rate)

    # plot mAP curve
    if len(val_map) != 0:
        from pytorch_ssd.plot_curve import plot_map
        plot_map(val_map)
...
if __name__ == '__main__':
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='../pytorch_Faster_RCNN', help='dataset')
    # 文件保存地址
