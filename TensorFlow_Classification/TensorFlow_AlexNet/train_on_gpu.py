...
"""
Author: LGD
FileName: train_on_gpu
DateTime: 2021/1/20 20:21 
SoftWare: PyCharm
"""
...
if gpus:
    try:
        for gpu in gpus:
            tf.config.experimental.set_memory_growth(gpu, True)  # 根据模型大小占用相应大小的显存。
...
# load train dataset
train_dataset = tf.data.Dataset.from_tensor_slices((train_image_list, train_label_list))  # 并行读取数据
