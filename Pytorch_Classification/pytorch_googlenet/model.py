...
class GoogLeNet(nn.Module):
    def __init__(self, num_class=1000, aux_logits=True, init_weights=False):
        super(GoogLeNet, self).__init__()
        self.aux_logits = aux_logits

        self.conv1 = BasicConv2d(3, 64, kernel_size=7, stride=2, padding=3)
        self.maxpool1 = nn.MaxPool2d(3, stride=2, ceil_mode=True)  # ceil_mode=True当计算结果是小数时，向上取整；当是False时，向下取整。

        self.conv2 = BasicConv2d(64, 64, kernel_size=1)
        self.conv3 = BasicConv2d(64, 192, kernel_size=3, padding=1)
        self.maxpool2 = nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True)

        self.inception3a = Inception(in_channel=192, ch1x1=64, ch3x3red=96, ch3x3=128, ch5x5red=16, ch5x5=32,
                                     pool_proj=32)
        ...
        if self.aux_logits:
            self.aux1 = InceptionAux(in_channel=512, num_classes=num_class)
            self.aux2 = InceptionAux(in_channel=128, num_classes=num_class)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))  # 自适应下采样，得到输出为（1×1）的矩阵
    ...
    def forward(self, x):
        ...
        x = torch.flatten(x, start_dim=1)  # N x 1024
        x = self.dropout(x)
...
class Inception(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.cat(outputs, 1)  # cat函数将四个输出进行合并，在深度上进行合并，dim=1
...
class InceptionAux(nn.Module):  # 辅助分类器
    def __init__(self, in_channel, num_classes):
        ...
        self.conv = BasicConv2d(in_channels=in_channel, out_channels=128, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # 当实例化一个模型model后，可以通过model.train()和model.eval()来控制模型的状态，
        # 在model.train()模式下，self.training=True,在model.eval()模式下self.training=False。
